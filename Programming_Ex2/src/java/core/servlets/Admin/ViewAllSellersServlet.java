...
@WebServlet(name = "ViewAllSellersServlet", urlPatterns = {"/viewallsellers"})
public class ViewAllSellersServlet extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        displayAllSellers(request, response);
    }

    private void displayAllSellers(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession session = request.getSession();
        String username = (String) session.getAttribute("username");

        ResultSet sellersData = DatabaseManager.getAllSellers();
        ...
        if (sellersData != null) {
            try {
                ...
                response.getWriter().println("<title>All Sellers</title>");
                ...
                response.getWriter().println("<h2>All Sellers</h2>");
                response.getWriter().println("<table>");
                response.getWriter().println("<tr><th>First Name</th><th>Last Name</th><th>Email</th><th>Sales</th><th>From A</th></tr>");

                while (sellersData.next()) {
                    String firstname = sellersData.getString("firstname");
                    String lastname = sellersData.getString("lastname");
                    String email = sellersData.getString("email");
                    String sales = sellersData.getString("sales");
                    String from_a = sellersData.getString("from_a");
                    response.getWriter().println("<tr><td>" + firstname + "</td><td>" + lastname + "</td><td>" + email + "</td><td>" + sales + "</td><td>" + from_a + "</td></tr>");
                }
            ...
            response.getWriter().println("</div>");
            response.getWriter().println("<div class=\"container\">");
            response.getWriter().println("<h2>No Sellers Found</h2>");
            response.getWriter().println("<a href=\"AdminPage.html\" class=\"back-button\">Go Back</a>");
            response.getWriter().println("</div>");
            response.getWriter().println("</body>");
            response.getWriter().println("</html>");
