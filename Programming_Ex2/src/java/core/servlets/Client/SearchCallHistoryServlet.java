...
@WebServlet(name = "SearchCallHistoryServlet", urlPatterns = {"/searchcallhistory"})
public class SearchCallHistoryServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<String> callHistory = DatabaseManager.searchCallHistoryByPhoneNumber(phoneNumber);
        ...
        response.getWriter().println("<title>Search Call History</title>");
        ...
        response.getWriter().println("<div class=\"user-name\">Welcome, " + emailCookie + "</div>");
        response.getWriter().println("<div class=\"header-buttons\">");
        response.getWriter().println("<form action=\"show-session\" method=\"post\" class=\"show-session-info\">");
        ...
        if (callHistory.size() != 0) {
            response.getWriter().println("<h2>Call History Found</h2>");
            response.getWriter().println("<table>");
            response.getWriter().println("<tr><th>Phone Number</th><th>Date</th></tr>");
            for (int i = 0; i < callHistory.size(); i += 2) {
                String phone = callHistory.get(i);
                String date = callHistory.get(i + 1);
                response.getWriter().println("<tr><td>" + phone + "</td><td>" + date + "</td></tr>");
            }
            response.getWriter().println("</table>");
        } else {
            response.getWriter().println("<h2>No Call History Found</h2>");
            System.out.println("No call history found for phone number: " + phoneNumber);
        }
        response.getWriter().println("<a href=\"ClientPage.html\" class=\"back-button\">Go Back</a>");
        response.getWriter().println("</div>");
        response.getWriter().println("</body>");
